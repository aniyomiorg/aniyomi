...
internal class AnimeExtensionInstallReceiver(private val listener: Listener) :
    ...
    private suspend fun getExtensionFromIntent(context: Context, intent: Intent?): AnimeLoadResult {
        val pkgName = getPackageNameFromIntent(intent)
            ?: return AnimeLoadResult.Error("Package name not found")
        return GlobalScope.async(Dispatchers.Default, CoroutineStart.DEFAULT) { AnimeExtensionLoader.loadExtensionFromPkgName(context, pkgName) }.await()
    }
