...
class MangaDownloader(
    ...
    fun clearQueue(isNotification: Boolean = false) {
        ...
        if (isNotification) {
            queue
                .filter { it.status == MangaDownload.State.QUEUE }
                .forEach { it.status = MangaDownload.State.NOT_DOWNLOADED }
        }
        queue.clear()
     ...
     */
    private fun downloadChapter(download: MangaDownload): Observable<MangaDownload> = Observable.defer {
     ...
     */
    private fun getOrDownloadImage(page: Page, download: MangaDownload, tmpDir: UniFile): Observable<Page> {
